...
class CouponBond:
    ...
    def at(self, x):
        ...
        for i in range(len(self.payTimes)):
            bond += self.cashFlows[i] * self.model.zeroBondPayoff(
                x, self.observationTime, self.payTimes[i]
            )
        return bond
...
class SwapRate:
    ...
    def at(self, x):
        ...
        for i in range(1, self.annuityTimes.shape[0]):
            annuity += (
                self.annuityTimes[i] - self.annuityTimes[i - 1]
            ) * self.model.zeroBondPayoff(x, self.observationTime, self.annuityTimes[i])
        floatLeg = self.model.zeroBondPayoff(
            x, self.observationTime, self.startTime
