...
class HullWhiteModel:
    ...
    def y(self, t):
        ...
        s1 = self.volatilityValues[
            min(idx + 1, len(self.volatilityValues) - 1)
        ]  # flat extrapolation
        y1 = (self.GPrime(t0, t) ** 2) * y0 + s1 ** 2 * (
            1.0 - np.exp(-2 * self.meanReversion * (t - t0))
        ) / (2.0 * self.meanReversion)
        return y1
    ...
    def couponBondOption(self, expiryTime, payTimes, cashFlows, strikePrice, callOrPut):
        ...
        xStar = brentq(
            objective, -1.0, 1.0, xtol=1.0e-8
        )  # +/-30% might be too narrow in some situations
        bondOption = 0.0
        for i in range(len(payTimes)):
            strike = self.zeroBond(expiryTime, xStar, payTimes[i])
            bondOption += cashFlows[i] * self.zeroBondOption(
                expiryTime, payTimes[i], strike, callOrPut
            )
        return bondOption

    # future yield curve in terms of forward rates
    def forwardRate(self, t, xt, T):
        return self.yieldCurve.forwardRate(T) + self.GPrime(t, T) * (
            xt + self.G(t, T) * self.y(t)
        )
...
class HullWhiteModelWithDiscreteNumeraire(HullWhiteModel):
    ...
    def __init__(self, yieldCurve, meanReversion, volatilityTimes, volatilityValues):
        HullWhiteModel.__init__(
            self, yieldCurve, meanReversion, volatilityTimes, volatilityValues
        )
