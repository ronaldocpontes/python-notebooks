...
class Swaption:
    ...
    def __init__(self, underlyingSwap, expiryDate, normalVolatility):
        ...
        self.swaption = ql.Swaption(
            self.underlyingSwap.swap, self.exercise, ql.Settlement.Physical
        )
        self.normalVolatility = normalVolatility
        volQuote = ql.SimpleQuote(normalVolatility)
        volHandle = ql.QuoteHandle(volQuote)
        initialEngine = ql.BachelierSwaptionEngine(
            self.underlyingSwap.discHandle, volHandle, ql.Actual365Fixed()
        )
    ...
    def npvRaw(self):
        ...
        CallOrPutOnS = (
            1.0 if self.underlyingSwap.payerOrReceiver == ql.VanillaSwap.Payer else -1.0
        )
        return self.annuity() * Bachelier(
            self.underlyingSwap.fixedRate,
            self.fairRate(),
            self.normalVolatility,
            T,
            CallOrPutOnS,
        )
    ...
    def vega(self):
        ...
        return (
            self.annuity()
            * BachelierVega(
                self.underlyingSwap.fixedRate, self.fairRate(), self.normalVolatility, T
            )
            * 1.0e-4
        )  # 1bp scaling
    ...
    def bondOptionDetails(self):
        ...
        details["callOrPut"] = (
            1.0
            if self.underlyingSwap.payerOrReceiver == ql.VanillaSwap.Receiver
            else -1.0
        )
        details["strike"] = 0.0
        refDate = self.underlyingSwap.discHandle.referenceDate()
        details["expiryTime"] = ql.Actual365Fixed().yearFraction(
            refDate, self.exercise.dates()[0]
        )
        fixedLeg = [
            [ql.Actual365Fixed().yearFraction(refDate, cf.date()), cf.amount()]
        ...
        floatLeg = [
            [
                ql.Actual365Fixed().yearFraction(
                    refDate, ql.as_coupon(cf).accrualStartDate()
                ),
                (
        ...
        payTimes = (
            ...
            + [
                ql.Actual365Fixed().yearFraction(
                    refDate,
                    ql.as_coupon(
                        self.underlyingSwap.swap.floatingLeg()[-1]
                    ).accrualEndDate(),
                )
            ]
        )
    ...
    def npvHullWhite(
        self, hwModel, outFlag="p"
    ):  # outFlag = [p]rice, [v]olatility or both [pv]
        details = self.bondOptionDetails()
        npv = hwModel.couponBondOption(
            details["expiryTime"],
            details["payTimes"],
            details["cashFlows"],
            details["strike"],
        ...
        if outFlag == "p":
            return npv
        vol = BachelierImpliedVol(
            npv / self.annuity(),
            self.underlyingSwap.fixedRate,
            self.underlyingSwap.fairRate(),
            details["expiryTime"],
            -details["callOrPut"],
        )
        if outFlag == "v":
    ...
    def payoff(self, hwModel):  # create a CouponBond payoff
        ...
        return CouponBond(
            hwModel, details["expiryTime"], details["payTimes"], cashFlowsCoP
        )
...
def HullWhiteModelFromSwaption(swaption, meanReversion=0.01):
    ...
    def objective(sigma):
        ...
        model = HullWhiteModel(
            swaption.underlyingSwap.discYieldCurve,
            meanReversion,
            volatilityTimes,
            volatilityValues,
        )
    ...
    sigma = brentq(
        objective, 0.01 * volatilityValues[0], 10.0 * volatilityValues[0], xtol=1.0e-8
    )
    volatilityValues[0] = sigma
...
class CashSettledSwaptionPayoff:
    ...
    def at(self, x):
        ...
        for cf in self.details["annuityLeg"]:
            annuity += cf[1] * self.model.zeroBondPayoff(
                x, self.details["expiryTime"], cf[0]
            )
        floatLeg = 0.0
        for cf in self.details[
            "floatLeg"
        ]:  # unfortunately, this only contains spread coupons
            floatLeg += cf[1] * self.model.zeroBondPayoff(
                x, self.details["expiryTime"], cf[0]
            )
        ...
        return (
            self.details["notional"]
            * cashAnnuity
            * self.details["callOrPut"]
