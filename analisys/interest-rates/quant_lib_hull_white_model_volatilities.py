...
for hwVol in hwVols:
    hwModel = HullWhiteModel(
        discCurve, meanReversion, np.array([30.0]), np.array([hwVol])
    )
    normalVols = [
        createSwaption(
            "10y", "10y", discCurve, projCurve, atmRate + strike
        ).npvHullWhite(hwModel, "v")
        * 1e4
...
def objective(sigma):
    tmpModel = HullWhiteModel(
        discCurve, meanReversion, np.array([30.0]), np.array([sigma])
    )
    return (
        createSwaption("10y", "10y", discCurve, projCurve).npvHullWhite(tmpModel, "v")
        - 0.01
    )  # we calibrate to 100bp 10y-10y vols
...
for expiry in expiries:
    for swapterm in swapterms:
        swaption = createSwaption(
            str(expiry) + "y", str(swapterm) + "y", discCurve, projCurve
        )
        vols[expiry - 1][swapterm - 1] = swaption.npvHullWhite(hwModel, "v") * 1e4
